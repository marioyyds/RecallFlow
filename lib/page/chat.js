...
const CSS = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: "Segoe UI", "Microsoft YaHei", system-ui, sans-serif; }
    ...
    .cmd-pick { border: 1px solid #c8d9ec; background: #f5f9fd; color: #3d6f9e; border-radius: 8px; padding: 0 10px; font-size: 12px; cursor: pointer; flex-shrink: 0; font-family: inherit; white-space: nowrap; }
...
// ---- 面板布局（大小/位置）本地持久化：写入 localStorage（随页面走，不做跨 tab 同步）----
...
function renumberCitations(text, citations) {
  ...
  if (!order.length) return { text, citations: list };
  // 无条件重编号为连续 1..N，保证正文一定从 [1] 开始。
  const oldToNew = {};
  order.forEach((old, i) => { oldToNew[old] = i + 1; });
  const replaceRe = /[\uFF3B\u005B]([^\]]*?)[\uFF3D\u005D]/g;
  ...
  // 参考来源区按「正文引用顺序」构建同样的 N 条：优先取编号匹配的来源；
  // 匹配不到时按注册顺序位置近似补齐，保证来源数量与正文一一对应且从 [1] 连续。
  // 注意：返回全新的对象副本，绝不改动传入的 citations（流式每 chunk 都会重编号，改动会破坏幂等）。
  const cited = order.map((old, k) => {
    const src = list.find((c) => Number(c.index) === old) || list[k] || { title: '来源 ' + (k + 1), url: '', snippet: '' };
    return Object.assign({}, src, { index: k + 1 });
  });
...
function attachSnippetParam(url, snippet) {
  try {
...
function renderAnswer(text, citations, streaming) {
...
function buildCitationDeepLink(url, snippet) {
  try {
...
function restoreStreamingUI() {
  ...
  for (const p of pendingParts) {
    if (p.type === 'narration' && p.text) {
      ...
      s.innerHTML = '<span class="agent-tool">🔧 ' + escHtml(p.name) + '</span>';
      flow.appendChild(s);
    } else if (p.type === 'tool-result') {
      const s = document.createElement('div');
      s.className = 'agent-step';
      s.textContent = (p.status === 'completed' ? '✓ 工具已完成：' : p.status === 'failed' ? '✗ 工具失败：' : '— 工具未执行：') + p.name;
      flow.appendChild(s);
    }
  }
      ...
      const flowHtml = parts
        .map((p) => {
          ...
          if (p.type === 'tool-result') {
            return '<div class="agent-step">' + (p.status === 'completed' ? '✓ 工具已完成：' : p.status === 'failed' ? '✗ 工具失败：' : '— 工具未执行：') + escHtml(p.name) + '</div>';
          }
          if (p.type === 'narration' && p.text) {
...
function serializeConversation() {
  return conversation.slice(-20).map((m) => {
...
function openPanel(x, y, docked) {
  ...
  panelBody.addEventListener('click', (e) => {
    ...
    if (cite) {
      ...
      if (url) {
        // 校验引用 URL 是否就是当前页面（同源同路径）；是则就地高亮，否则跨页跳转。
        let samePage = false;
        try {
          const u = new URL(url);
        ...
        if (samePage) {
          ...
          // 跨页：深链打开（Chrome 文本片段原生高亮）+ 附带 kbSnippet 参数，
          // 目标页的内容脚本会用更容错的字符级匹配再高亮一次，双保险。
          window.open(attachSnippetParam(buildCitationDeepLink(url, snippet), snippet), '_blank', 'noopener');
        }
      } else if (id) {
  ...
  getAISettings().then((s) => { const p = s.toolApprovalPolicy || { read: true }; approvalSettings.querySelectorAll('[data-policy]').forEach((x) => { x.checked = p[x.dataset.policy] === true; x.addEventListener('change', async () => { const policy = Object.fromEntries([...approvalSettings.querySelectorAll('[data-policy]')].map((y) => [y.dataset.policy, y.checked])); await chrome.storage.local.set({ aiSettings: { ...s, toolApprovalPolicy: policy } }); autopilotEl.checked = ['read', 'edit', 'commands', 'browser', 'mcp'].every((k) => policy[k] === true); refreshApprovalLabel(); }); }); autopilotEl.checked = ['read', 'edit', 'commands', 'browser', 'mcp'].every((k) => p[k] === true); refreshApprovalLabel(); }).catch(() => {});
  ...
  pickBtn.textContent = '⊕ 选元素';
  pickBtn.title = '在页面上点选一个元素，把它和对应的前端源码位置一起发给 AI（Esc 取消）';
  ...
  const renderPick = () => {
    ...
    const loc = pickedElement.source && pickedElement.source.file
      ? pickedElement.source.file + (pickedElement.source.line ? ':' + pickedElement.source.line : '')
      : '';
    const chip = document.createElement('span');
    chip.className = 'pick-chip';
    chip.textContent =
      '已选：' + (pickedElement.tag || '') +
      (pickedElement.label ? '「' + pickedElement.label + '」' : '') +
      (loc ? ' → ' + loc + (pickedElement.source.component ? '（' + pickedElement.source.component + '）' : '') : '（未检测到框架源码）');
  ...
  function run(instruction, opts) {
    ...
    conn.onMessage.addListener((resp) => {
      ...
      if (resp.type === 'citations') {
        ...
        const step = document.createElement('div');
        step.className = 'agent-step';
        step.textContent = resp.status === 'completed' ? '✓ 工具已完成：' + resp.name : resp.status === 'failed' ? '✗ 工具失败：' + resp.name : '— 工具未执行：' + resp.name;
        appendStep(step);
        fitPanelHeight();
        if (observerAutoScroll && panelBody) panelBody.scrollTop = panelBody.scrollHeight;
        ...
        step.textContent = resp.ok
          ? '✓ 完成前校验通过' + (resp.reason ? '：' + resp.reason : '')
          : '⚠ 校验未通过：' + (resp.reason || '目标可能未达成') + (resp.missing ? '（还差：' + resp.missing + '）' : '');
        appendStep(step);
        fitPanelHeight();
        if (observerAutoScroll && panelBody) panelBody.scrollTop = panelBody.scrollHeight;
        ...
        pendingParts.push({ type: 'text', text: pendingNarration ? finalText : '' });
        ...
        if (lastResponse) pendingAiMsg.insertAdjacentHTML('beforeend', msgActionsHtml());
        pendingParts.push({ type: 'text', text: lastResponse });
        conversation.push({ role: 'assistant', content: finalText, citations: finalCitations, plan: lastPlan.slice(), parts: pendingParts.map((p) => (p.type === 'text' && p.text === lastResponse ? Object.assign({}, p, { text: finalText }) : p)) });
        saveConversation();
