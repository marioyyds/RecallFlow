...
// 会话存储拆分两个 key，各自由单一写入方「整份 set」写入（不做 get+set 合并，避免并发覆盖丢数据）：
//   kbConversation = { list, ts }  对话内容（含发起 tab 节流写入的当前轮累积），仅 saveConversation 写入；
//   kbPanelState   = { panelOpen, panelLayout, ts }  面板开关/布局，仅 setPanelState 写入；
//   kbSessionTab   = { tabId, ts }  Agent 当前绑定标签页（备用，与面板状态解耦，见 tools.js bindSessionTab）。
const CONV_KEY = 'kbConversation';
const PANEL_KEY = 'kbPanelState';
const SESSION_TAB_KEY = 'kbSessionTab';
// 最近 30 分钟内未结束的会话，跨页面跳转 / 新标签页自动恢复对话面板。
const RESTORE_WINDOW_MS = 30 * 60 * 1000;

// 当前标签页自身的 tabId（通过 background 查询获取，供会话「弹窗跟随」判断）。
let ownTabId = null;
...
function removePanel() {
  ...
  clearCiteHighlight();
  if (panel) {
    // 会话面板状态置为关闭，跨 tab 同步（storage 事件广播）。
    try { setPanelState({ panelOpen: false }); } catch (e) {}
...
// ---- 会话/面板状态写入：单一写入方 + 整份快照 ----
// 关键：对话内容「整份 set」写入，不做 get+set 合并——否则两个 saveConversation 并发时，
// 后到的 set 基于更旧的 get 结果会把更新的对话覆盖掉，导致「丢历史」。
// 观察者 tab 通过 chrome.storage.onChanged 事件即时刷新（见 onStorageChanged），无需轮询。
// 面板状态（开关/布局）以内存缓存为准、整份写入，避免与 bindSessionTab 等其它写入方互相覆盖。
let panelState = { panelOpen: false, panelLayout: null, ts: 0 };
function setPanelState(patch) {
  try {
    panelState = Object.assign({}, panelState, patch);
    panelState.ts = Date.now();
    chrome.storage.local.set({ [PANEL_KEY]: panelState });
  } catch (e) {}
}

// ---- 面板布局（大小/位置）跨 tab 同步 ----
let layoutTimer = null;
let cachedPanelLayout = null; // 本 tab 缓存的布局，openPanel 时同步应用，避免依赖异步时序

// 把当前面板的布局写入会话对象（节流，避免 mousemove 高频写 storage）。
...
function persistPanelLayoutNow() {
  ...
  setPanelState({ panelLayout: layout });
}

// 把会话里的布局应用到本 tab 面板（clamp 到本 tab 视口内，不同窗口视口大小可能不同）。
function applyPanelLayout(layout) {
  if (!layout || !panel) return;
  cachedPanelLayout = layout;
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const w = Math.min(vw - 24, Math.max(280, Number(layout.width) || 320));
  const h = Math.min(vh - 24, Math.max(320, Number(layout.height) || 400));
  let left = Math.min(Math.max(0, Number(layout.left) || 0), Math.max(0, vw - w));
  let top = Math.min(Math.max(0, Number(layout.top) || 0), Math.max(0, vh - h));
...
// 判断当前会话弹窗是否「打开且有内容且新鲜」：全局同步——任何 tab 都据此显示/隐藏。
// 需要同时满足：面板状态打开 + 有对话内容 + 最近 30 分钟内有活动（对话或面板激活）。
function shouldShowSession(conv, state) {
  const ts = Math.max(Number(conv && conv.ts || 0), Number(state && state.ts || 0));
  return Boolean(
    state &&
      state.panelOpen === true &&
      conv &&
      Array.isArray(conv.list) &&
      conv.list.length &&
      Date.now() - ts < RESTORE_WINDOW_MS
  );
}

// 单一写入方：对话内容（含「进行中」的当前轮）统一写入 kbConversation。
// 流式进行中时，把当前轮已累积的 pendingNarration 作为一条 _streaming 临时 assistant 一并写入，
// 观察者 tab 直接读 list 即可看到完整历史 + 当前轮实时进度。
// 结束/中断时 pendingNarration 已清空（endStream / 中断前手动清空），不会再重复追加。
function saveConversation() {
  try {
    const list = conversation.slice(-50);
    if (streaming && pendingNarration) {
      list.push({ role: 'assistant', content: pendingNarration, _streaming: true });
    }
    chrome.storage.local.set({ [CONV_KEY]: { list, ts: Date.now() } });
  } catch (e) {}
}

// 流式 token 高频，节流写入（400ms）避免每字一次 storage 写；统一走 saveConversation。
let persistConvTimer = null;
function persistConversationThrottled() {
  if (persistConvTimer) return;
...
// 清空节流定时器（对话固化到 list 前调用，避免残留写入覆盖正式内容）。
function clearPersistConvTimer() {
  if (persistConvTimer) { clearTimeout(persistConvTimer); persistConvTimer = null; }
}

function loadConversation() {
  return new Promise((resolve) => {
    chrome.storage.local.get([CONV_KEY, PANEL_KEY], (d) => {
      const conv = d[CONV_KEY] || {};
      const state = d[PANEL_KEY] || {};
      // 用 storage 中的面板状态初始化内存缓存，后续 setPanelState 才不会再把它覆盖成默认值。
      panelState = {
        panelOpen: state.panelOpen === true,
        panelLayout: state.panelLayout || null,
        ts: Number(state.ts) || 0,
      };
      if (shouldShowSession(conv, state) && Array.isArray(conv.list)) {
        conversation = conv.list.slice();
      } else {
        conversation = [];
      }
      resolve();
...
// 在 list 同步（观察者 / 历史恢复）后统一调用，避免「会话结束后打开」时无按钮。
...
// 当前面板的发送函数（openPanel 里绑定 run），供顶层 renderSuggestionButtons 在
// 发起 tab 与观察者 tab 都能触发「继续」。
let activeSendFn = null;

// 解析触底建议中的「可选操作」，返回 [{label, desc}]（最多 4 条）。
// 优先解析 <options>JSON</options> 程序块；失败再回退解析 【可选操作】 小节的文本格式。
// 两种情况都解析不到则返回空数组，前端自然退化为纯文本，不报错。
function parseSuggestionOptions(text) {
  const out = [];
...
// 发起 tab（run() 的 end 处理）与观察者 tab（轮询同步）共用。
// sendFn 用于绑定「所属面板」的发送函数，避免多 tab 时串到其他面板。
...
function renderConversation() {
  ...
  panelBody.innerHTML = conversation
    .map((m, index) => {
      ...
      const flowHtml = parts
        .map((p) => {
          ...
          if (p.type === 'narration' && p.text) {
            return '<div class="agent-content">' + renderMarkdown(stripOptionsBlock(p.text), null, false) + '</div>';
          }
      ...
      const contentHtml = hasInlineText ? '' : renderMarkdown(stripOptionsBlock(m.content || ''), m.citations);
      return (
...
function openPanel(x, y, docked) {
  ...
  panelBody.addEventListener('scroll', () => {
    ...
    observerAutoScroll = panelBody.scrollHeight - panelBody.scrollTop - panelBody.clientHeight < 40;
  });
  panelBody.addEventListener('click', (e) => {
    const rewind = e.target.closest('.act-rewind');
      if (rewind) {
        ...
        if (Number.isInteger(index) && conversation[index] && conversation[index].role === 'user') {
          ...
          renderConversation();
          fitPanelHeight();
          if (observerAutoScroll) panelBody.scrollTop = panelBody.scrollHeight;
    ...
    if (cite) {
      ...
      if (source === 'page') {
        ...
        // 校验引用是否来自当前页面（同源同路径）；跨页/跳转后不强行在错误页面定位。
        const citeUrl = cite.getAttribute('data-cite-url') || '';
        let samePage = false;
        if (citeUrl) {
          try {
          ...
          } catch (e) { samePage = false; }
        }
        if (!samePage) {
          showCitationHint('该引用来自其他页面或当前页已跳转，无法在此高亮。');
          return;
        }
        const found = findAndHighlightCitation(snippet);
        if (!found) showCitationHint('未能在当前页面定位该引用（内容可能已变化或尚未加载）。');
      } else if (url) {
  ...
  // 跨 tab 同步由 chrome.storage.onChanged 事件驱动（见 onStorageChanged），此处无需轮询。
  ...
  // 会话面板置为打开，跨 tab 全局同步（storage 事件广播到所有 tab）。
  // 同时刷新会话时间戳，视为重新激活（过期会话不再被其他 tab 弹出）。
  try { setPanelState({ panelOpen: true }); } catch (e) {}
  ...
  // 应用布局（大小/位置）：优先本 tab 缓存（maybeAutoRestorePanel 预置或本 tab 刚调整过），
  // 再异步读 storage 兜底覆盖——确保新开 tab 面板立即落到同步的位置/大小。
  if (cachedPanelLayout) applyPanelLayout(cachedPanelLayout);
  chrome.storage.local.get(PANEL_KEY, (d) => {
    const s = d[PANEL_KEY] || {};
    if (s.panelLayout) applyPanelLayout(s.panelLayout);
  });
    ...
    closePort();
      if (pendingAiMsg && pendingAiMsg.parentNode) {
        if (acc) {
          ...
          // 当前轮已固化进 conversation（content=acc 含完整文本），清掉临时流式状态避免重复追加 _streaming 消息。
          pendingNarration = '';
          clearPersistConvTimer();
          saveConversation();
  ...
  function run(instruction, opts) {
    ...
    // 立即持久化用户指令，保证其他 tab（观察者）打开面板时能看到「之前的历史」，
    // 而不是只看到当前流式。
    saveConversation();
      ...
      if (resp.type === 'citations') {
        ...
        if (resp.requiresApproval) {
          const approvalArgs = resp.name === 'type_text' && resp.args
            ...
            : resp.name === 'run_javascript' && resp.args
              ? { ...resp.args, code: String(resp.args.code || '').slice(0, 400) + (String(resp.args.code || '').length > 400 ? '…（代码已截断）' : '') }
              : resp.args;
          ...
          // 高风险工具（run_javascript）不允许“本次会话允许”，避免一次放行长期免审。
          const sessionBtn = resp.name === 'run_javascript'
            ? ''
            : '<button class="tool-approve-session" aria-label="本次会话允许此工具">本次会话允许</button>';
        ...
        if (pendingContentEl) pendingContentEl.innerHTML = renderMarkdown(stripOptionsBlock(pendingNarration), currentCitations, true);
        aiMsg._raw = pendingAcc;
        // 节流把「完整对话（含当前轮累积）」写入 storage，观察者 tab 直接读 list 即可看到。
        ...
        if (!pendingStarted) {
          ...
          pendingContentEl.innerHTML = renderMarkdown(stripOptionsBlock(pendingNarration || lastResponse), currentCitations, false);
          aiMsg._raw = pendingAcc;
        }
        ...
        // 当前轮已固化进 conversation，先清掉临时流式状态，避免 saveConversation 重复追加 _streaming 临时消息。
        pendingNarration = '';
        clearPersistConvTimer();
        // 触底收尾：解析模型输出的「【可选操作】」小节，渲染可点击继续按钮。
        renderSuggestionButtons(aiMsg, lastResponse, run);
        saveConversation();
        fitPanelHeight();
  ...
  // 绑定当前面板的发送函数到面板元素，供「决策建议按钮」（发起/观察者）触发继续，
  // 避免多 tab 时串到其他面板。
  panel._sendFn = run;
  activeSendFn = run;
...
// 自动恢复对话面板：页面跳转后，若当前会话已绑定到本 tab、面板为打开状态且会话新鲜，
// 则自动弹出面板并恢复历史，让对话跨页面/新 tab 延续。
export function maybeAutoRestorePanel() {
  chrome.storage.local.get([CONV_KEY, PANEL_KEY], (d) => {
    if (panel) return;
    const conv = d[CONV_KEY] || {};
    const state = d[PANEL_KEY] || {};
    if (!shouldShowSession(conv, state)) return;
    // 预置布局缓存，openPanel 创建面板时同步应用（避免异步时序导致位置先落默认值再跳变）。
    if (state.panelLayout) cachedPanelLayout = state.panelLayout;
    // 延迟片刻再弹，避免页面一进来就抢焦点、显得突兀。
    setTimeout(() => {
      if (panel) return;
      ensureHost();
      openPanel(0, 0, true);
...
// 查询当前标签页自身的 tabId（缓存），供会话「弹窗跟随」判断。
function getOwnTabId() {
  return new Promise((resolve) => {
    chrome.runtime.sendMessage({ type: 'getTabId' }, (res) => {
      ownTabId = res && res.tabId ? res.tabId : null;
      resolve(ownTabId);
    });
  });
}
...
// ---- 流式跨 tab 观察者渲染 ----
// 观察者面板是否自动跟随底部：用户手动向上滚动（查看历史）时置 false，滚回底部恢复。
let observerAutoScroll = true;

// ---- 观察者事件驱动同步（替代轮询/广播）----
// 发起 tab 仍是逐字实时（port）；其它 tab 监听 chrome.storage.onChanged，
// 任一写入（对话内容 / 面板开关布局）即时刷新，可靠、不丢、不截断、无延迟。
// 发起 tab 自己忽略对话的 onChanged（它有 port 实时流），只在其作为观察者时响应其它会话。
function syncConversationFromStorage(conv) {
  if (streaming) return; // 发起 tab 走 port 实时，不从 storage 重渲染
  if (!panel) return;
  if (!Array.isArray(conv.list) || !conv.list.length) return;
  const same =
    conversation.length === conv.list.length &&
    conversation.every((m, i) => conv.list[i] && m.role === conv.list[i].role && m.content === conv.list[i].content);
  // 用户手动上滚查看历史时不打断；仅内容确有变化才重建。
  if (!same && observerAutoScroll) {
    conversation = conv.list.slice();
    renderConversation();
    fitPanelHeight();
    renderDecisionButtonsIfPresent();
...
function syncPanelStateFromStorage(state) {
  // 面板开关同步（全局：任一 tab 关闭则全部关闭）
  if (state.panelOpen !== true) {
    removePanel();
    return;
  }
  // 布局同步：拖拽/拉伸进行中不覆盖，避免抖动
  if (state.panelLayout && panel && !dragState && !resizeState) {
    applyPanelLayout(state.panelLayout);
  }
  // 其它 tab 打开了面板而本 tab 还没开 → 自动恢复
  if (!panel) maybeOpenFromState(state);
}

// 被动 tab 在「发起 tab 已开面板、但本 tab 后加载」等场景下补开面板（无延迟、即时）。
function maybeOpenFromState(state) {
  if (panel || state.panelOpen !== true) return;
  if (state.panelLayout) cachedPanelLayout = state.panelLayout;
  ensureHost();
  openPanel(0, 0, true);
}
...
function onStorageChanged(changes, area) {
  ...
  if (changes[CONV_KEY]) {
    syncConversationFromStorage(changes[CONV_KEY].newValue || {});
  }
  if (changes[PANEL_KEY]) {
    syncPanelStateFromStorage(changes[PANEL_KEY].newValue || {});
  }
}
...
export function initAssistant() {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    if (dragState && panel) {
      ...
      // 大小/位置变化节流写入会话，供其他 tab 同步布局。
      persistPanelLayoutThrottled();
    }
  });
  document.addEventListener('mouseup', () => {
    // 拖拽/拉伸结束：立即持久化最终布局（供其他 tab 同步）。
  ...
  ensureHost();
  // 监听会话变化的存储事件，实现弹窗开关与对话内容的跨 tab 全局同步（事件驱动，无需轮询）。
  if (chrome.storage && chrome.storage.onChanged) {
    chrome.storage.onChanged.addListener(onStorageChanged);
  }
  getOwnTabId().then(() => {
    maybeAutoRestorePanel();
  });
}
