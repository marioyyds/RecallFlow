...
const ACTIVE_SESSION_KEY = 'kbActiveSession';
...
function removePanel() {
  ...
  if (panel) {
    ...
    try { updateSession({ panelOpen: false }); } catch (e) {}
...
// 会话对象写入（合并式更新）：{ url, list, ts, active, panelOpen, tabId, startedByTabId }。
// 所有 tab 通过 chrome.storage.onChanged 事件感知会话变化，实现弹窗/对话跨 tab 同步。
// 值无变化时不写，避免多 tab 间的 onChanged 事件循环。
function updateSession(patch) {
  try {
    chrome.storage.local.get(ACTIVE_SESSION_KEY, (d) => {
      const s = d[ACTIVE_SESSION_KEY] || {};
      const next = Object.assign({}, s, patch);
      if (JSON.stringify(next) === JSON.stringify(s)) return;
      chrome.storage.local.set({ [ACTIVE_SESSION_KEY]: next });
    });
  } catch (e) {}
}

// 判断当前会话弹窗是否「打开且有内容且新鲜」：全局同步——任何 tab 都据此显示/隐藏。
// 不再按 tab 归属判断，保证弹窗开关状态跨 tab 一致。
function shouldShowSession(session) {
  return Boolean(
    session &&
      ...
      session.list.length &&
      Date.now() - Number(session.ts || 0) < RESTORE_WINDOW_MS
  );
}
...
function saveConversation() {
  try {
    ...
    updateSession({ list, ts: Date.now(), active: streaming, url: location.href });
  } catch (e) {}
}
...
function loadConversation() {
  return new Promise((resolve) => {
    chrome.storage.local.get(ACTIVE_SESSION_KEY, (d) => {
      const s = d[ACTIVE_SESSION_KEY] || {};
      if (shouldShowSession(s) && Array.isArray(s.list)) {
        conversation = s.list.slice();
      } else {
        conversation = [];
      }
      resolve();
...
function openPanel(x, y, docked) {
  ...
  try { updateSession({ panelOpen: true, ts: Date.now() }); } catch (e) {}
  ...
  function renderSuggestionButtons(container, text) {
    ...
    options.forEach((opt) => {
      ...
      b.addEventListener('click', () => {
        if (streaming) return;
        run(opt.label + (opt.desc ? '：' + opt.desc : ''), { continuation: true });
      });
      wrap.appendChild(b);
    });
  ...
  function run(instruction, opts) {
    ...
    conn.onMessage.addListener((resp) => {
      ...
      if (resp.type === 'citations') {
        ...
        renderSuggestionButtons(aiMsg, lastResponse);
        saveConversation();
        fitPanelHeight();
...
export function maybeAutoRestorePanel() {
  chrome.storage.local.get(ACTIVE_SESSION_KEY, (d) => {
    if (panel) return;
    if (!shouldShowSession(d[ACTIVE_SESSION_KEY])) return;
...
// 会话变化事件驱动同步（chrome.storage.onChanged 广播到所有 tab）：
// 全局同步——panelOpen=true 时任何 tab 都显示弹窗并同步对话内容；
// panelOpen=false 时任何 tab 都关闭弹窗。正在流式的 tab 不因其他 tab 的关闭而被强制中断。
function syncFromSession(session) {
  // 全局同步：panelOpen=true 且会话新鲜 → 任何 tab 都显示弹窗并同步内容；否则关闭。
  const open = Boolean(session && session.panelOpen === true && Date.now() - Number(session.ts || 0) < RESTORE_WINDOW_MS);
  if (open) {
    if (!panel) {
      ensureHost();
      openPanel(0, 0, true);
    }
    // 非发起 tab（无流式）时，同步对话内容；发起 tab 正在流式时交给流式渲染，避免打断。
    if (!streaming && panelBody && Array.isArray(session.list)) {
      const same =
        conversation.length === session.list.length &&
        conversation.every((m, i) => session.list[i] && m.role === session.list[i].role && m.content === session.list[i].content);
      if (!same) {
        conversation = session.list.slice();
        renderConversation();
        fitPanelHeight();
      }
    }
  ...
  // 全局关闭：不中断正在流式的对话 tab（流式结束后的下次事件会再关闭）。
  if (panel && !streaming) {
    removePanel();
  }
}
...
export function initAssistant() {
  ...
  ensureHost();
  // 监听会话变化的存储事件，实现弹窗开关与对话内容的跨 tab 全局同步。
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== 'local' || !changes[ACTIVE_SESSION_KEY]) return;
    syncFromSession(changes[ACTIVE_SESSION_KEY].newValue || null);
  });
  getOwnTabId().then(() => {
