...
const TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    function: {
      name: 'read_network',
      description: '读取当前页面最近的网络请求（fetch / XHR：URL、方法、状态码、耗时、错误），用于前端调试。可选 URL 子串过滤与 limit。',
      parameters: {
        type: 'object',
        properties: {
  ...
  switch (name) {
    ...
    case 'read_network': {
      ...
      try {
        ...
        const text = entries
          .map((e) => (e.method || 'GET') + ' ' + (e.error ? 'ERR' : (e.status || '')) + ' ' + e.url + (e.ms != null ? ' (' + e.ms + 'ms)' : '') + (e.error ? ' — ' + e.error : ''))
          .join('\n');
