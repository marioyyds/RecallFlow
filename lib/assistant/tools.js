...
const TOOL_METADATA = {
  ...
  web_search: { risk: 'network', readOnly: true, requiresApproval: false, route: 'background' },
...
- 读取当前浏览的网页正文（read_current_page）或结构化 DOM 快照（get_page_snapshot）、列出/打开/切换浏览器标签页、网络搜索（web_search）、抓取外部网页（fetch_webpage）；
  ...
  switch (name) {
    ...
    case 'web_search': {
      ...
      try {
        ...
        const citations = results.map((r, i) => ({ index: i + 1, source: 'web', title: r.title, url: r.url, snippet: (r.snippet || '').slice(0, 180) }));
        return { result: '搜索结果（' + q + '）：\n' + lines.join('\n\n'), citations };
      } catch (e) {
...
async function searchWeb(query, maxResults = 6) {
  ...
  const engines = [
    ...
    { name: 'bing', url: 'https://www.bing.com/search?q=' + q + '&setlang=zh-hans' },
  ];
  for (const engine of engines) {
    try {
      const ctrl = typeof AbortController !== 'undefined' ? new AbortController() : null;
      const timer = ctrl ? setTimeout(() => ctrl.abort(), 15000) : null;
      const resp = await fetch(engine.url, { method: 'GET', redirect: 'follow', signal: ctrl ? ctrl.signal : undefined });
      if (timer) clearTimeout(timer);
      if (!resp.ok) continue;
      const ct = (resp.headers.get('content-type') || '').toLowerCase();
