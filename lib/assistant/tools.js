...
// 把 Agent 当前绑定的标签页写入独立 key（kbSessionTab），与「面板状态 kbPanelState」解耦，
// 避免与面板开关/布局的写入方在 get+set 合并时互相覆盖（导致面板状态丢失）。
// tabId 仅备用，不参与跨 tab 弹窗同步逻辑。
